export type RecursiveEnumObjectType = {
    [key: string]: number | RecursiveEnumObjectType;
};
...
type __Flatten<T, Prefix extends string = ""> = {
    [K in keyof T]: T[K] extends RecursiveEnumObjectType
        ? __Flatten<T[K], Prefix extends "" ? `${string & K}` : `${Prefix}.${string & K}`>
        : Prefix extends ""
          ? { [Key in `${string & K}`]: T[K] }
...
type Flatten<T, Prefix extends string = ""> = ValueOf<{
    [K in keyof T]: T[K] extends RecursiveEnumObjectType
        ...
        */
export type Flatten<T extends RecursiveEnumObjectType> =
...
export function flatten<T extends RecursiveEnumObjectType>(
    recursiveEnumObject: T,
): Flatten<typeof recursiveEnumObject> {
    function helper<T extends RecursiveEnumObjectType>(obj: T): Flatten<T> {
